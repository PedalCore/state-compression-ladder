...
def train_one(k, d, Str, Sval, Ste, dev, epochs, use_wandb=False,
    ...
    res = dict(k=k, seed=seed, params=nprm,
               v_rmse_mv=round(vr, 2), gate_rmse=round(gr, 4),
               spike_f1=round(f1, 3), fi_rmse_hz=round(fi_rmse, 1),
               rebound_spikes=reb)
    print('RESULT', json.dumps(res), flush=True)
    torch.save(model.state_dict(), OUT / f'mech_k{k}_s{seed}.pt')
    return res
...
def main():
    ...
    from hh_teacher import init_state
    ...
    results = [train_one(int(k), d, Str, Sval, Ste, args.dev,
                         args.epochs, use_wandb, seed=int(sd))
               for k in args.ks.split(',')
               for sd in args.seeds.split(',')]
    tag = 'mech_results' + ('' if args.seeds == '0'
                            else '_seeds')
    json.dump(results, open(OUT / f'{tag}.json', 'w'), indent=1)
